...
\begin{proof}
  ...
  t[x := v]$, so we can conclude using Lemma~\ref{lem:eqred}. Note that it is
  important that $v ≠ □$ so that the second reduction step can be taken.
\end{proof}
...
\end{theorem}
\begin{proof}
  We have ${\{(l_i = v_i)_{i∈I}\}.l_k ∗ π} ≻ {v_k ∗ π}$ since $k ∈ I$, and
  hence we can conclude by Lemma~\ref{lem:eqred}.
\end{proof}
...
\begin{proof}
  ...
  since $k ∈ I$, and we can hence conclude using Lemma~\ref{lem:eqred}.
\end{proof}

% Fixpoint step
We have ${φa.v ∗ π} ≻ {v[a := φa.v] ∗ π}$.

% Record filter
We have ${R(\{(l_i = v_i)_{i∈I}\},t) ∗ π} ≻ {t ∗ π}$.

% Application with box
We have ${t\;□ ∗ π} ≻ {□ ∗ [t\;\,{-}]π} ≻ {□ ∗ π}$. And similarly, we have
${□\;v ∗ π} ≻ {v ∗ [□\;\,{-}]π} ≻ {v ∗ π}$. Note that in the last step, a
different rule applies depending whether $v = □$ or $v ≠ □$.

% Box projection
We have ${□.l_k ∗ π} ≻ {□ ∗ π}$.

% Box matching
We have ${[□\,| (C_i[x_i] → t_i)_{i∈I}] ∗ π} ≻ {□ ∗ π}$.
