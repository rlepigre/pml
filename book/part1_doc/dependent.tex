...
Then, in the type \dupml{st⟨a,c⟩}, we check that the ordering is respected
...
The module \dupml{lib.comparison} define the functions
\dupml{is_Ls} and \dupml{is_Gr} to check the return
value of the function \dupml{c : a ⇒ a ⇒ [ Ls; Eq; Gr]}.

Now, we can define the function doing insertion in a binary search tree:
\begin{pmlcode}
val rec insert : ∀o,∀a, (a ⇒ a ⇒ cmp) ⇒ a ⇒ bt⟨a⟩ ⇒ bt⟨a⟩ =
  fun c e t {
    case t {
      | []                  → Node[{ l = []; e; r = []}]
      | Node[{ l; e=e'; r}] →
...
val rec insert2 : ∀o,∀a, ∀c∈(a ⇒ a ⇒ cmp), ∀e∈a, ∀t∈st⟨a,c⟩,
                 insert c e t ∈ st⟨a,c⟩ =
  fun c e t {
    case t {
      []                 → Node[{ l = []; e; r = []}]
