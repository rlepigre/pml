...
val uu : ∀ a:ο→ο→ο, (∀ x y:ο, a<x,y>) ⇒ (∀ x y:ο, a<y,x>) = λx.x
val ee : ∀ a:ο→ο→ο, (∃ x y:ο, a<x,y>) ⇒ (∃ x y:ο, a<y,x>) = λx.x
val eu : ∀ a:ο→ο→ο, (∃ x:ο, ∀ y:ο, a<x,y>) ⇒ (∀ y:ο, ∃ x:ο, a<x,y>) = λx.x

// Invalid.
// val ue : ∀ a:ο→ο→ο, (∀ x:ο, ∃ y:ο, a<x,y>) ⇒ (∃ y:ο, ∀ x:ο, a<x,y>) = λx.x

// Invalid.
// val pr : ∀ a b:ο→ο, (∀ x:ο, {l : a<x>; r : b<x>})
//        ⇒ {l : ∀ x:ο, a<x>; r : ∀ x:ο, b<x>} = λx.x

val pr : ∀ a b:ο→ο, {l : ∀ x:ο, a<x>; r : ∀ x:ο, b<x>}
       ⇒ (∀ x:ο, {l : a<x>; r : b<x>}) = λx.x

// Invalid.
// val sm : ∀ a b:ο→ο, (∀ x:ο, [L of a<x> ; R of b<x>])
//        ⇒ [L of ∀ x:ο, a<x>; R of ∀ x:ο, b<x>] = λx.x

val sm : ∀ a b:ο→ο, [L of ∀ x:ο, a<x>; R of ∀ x:ο, b<x>]
       ⇒ (∀ x:ο, [L of a<x> ; R of b<x>]) = λx.x
