...
let to_valu : boxed -> v ebox = sort_filter V

let to_term : boxed -> t ebox = fun e ->
  match e with
  | Box(T,e) -> e
  | Box(V,e) -> valu (Bindlib.unbox e).pos e
  | _        -> assert false

let to_stac : boxed -> s ebox = sort_filter S
let to_prop : boxed -> p ebox = sort_filter P
let to_ordi : boxed -> o ebox = sort_filter O
